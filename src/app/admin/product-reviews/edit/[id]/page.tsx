...
export default function EditProductReview({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [cats, auths, exps, reviewRes] = await Promise.all([
                    fetch('/api/review-categories').then(res => res.json()),
                    fetch('/api/authors').then(res => res.json()),
                    fetch('/api/experts').then(res => res.json()),
                    fetch(`/api/product-reviews?slug=${id}`).then(res => res.json()) // Note: API currently uses slug for unique fetch
                ]);

                // If slug fetch fails, try ID fetch if API supports it (checking route.ts it seems GET uses slug)
                // Actually the route currently only has slug fetch for individual reviews
                // I'll update the API later if needed, but for now I'll use what's there
                let review = reviewRes;
                if (reviewRes.error) {
                    // Fallback search by ID if slug is actually ID
                    const allRes = await fetch('/api/product-reviews').then(res => res.json());
                    review = allRes.find((r: any) => r.id === id);
                }
                ...
                if (review) {
                    ...
                    setGuarantee(review.guarantee || '');
                    setSideEffects(review.sideEffects || '');
                    setDescription(review.description || '');
                    setTargetAudience(review.targetAudience || '');
                    setWorkingMechanism(review.workingMechanism || '');
                    setSafetyInfo(review.safetyInfo || '');
                    setWarnings(review.warnings || '');
                    setRecommendation(review.recommendation || '');
                    setAffiliateLink(review.affiliateLink || '');
                    setFeaturedImage(review.featuredImage || '');
                    setContent(review.content || '');
                    setFinalRating(review.finalRating || 0);
                    setAuthorId(review.authorId || '');
                    setCategoryId(review.categoryId || '');
                    setMedicalReviewerId(review.medicalReviewerId || '');
                    setPros(review.pros?.length > 0 ? review.pros : ['']);
                    setCons(review.cons?.length > 0 ? review.cons : ['']);
                    setIngredients(review.ingredients?.length > 0 ? review.ingredients : ['']);
                    setBenefitsList(review.benefitsList?.length > 0 ? review.benefitsList : ['']);
                    setDetailedIngredients(review.detailedIngredients || []);
                    setCustomerReviews(review.customerReviews || []);
                    setPricingOffers(review.pricingOffers || []);
                    setFaqs(review.faqs || []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            id, productName, slug, rating: Number(rating), verdict, brand, form, price,
        ...
        <div className="max-w-7xl mx-auto pb-20 px-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase">Edit Product Review</h1>
                        <p className="text-gray-500 font-medium">Refining the clinical deep dive analysis</p>
                    </div>
                </div>
                <div className="flex gap-4 w-full md:w-auto">
                    ...
                    >
                        {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Update Review
                    </button>
                </div>
            </div>
                    ...
                    <section className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex flex-wrap items-center gap-6">
                                <Link href={`/reviews/${slug}`} target="_blank" className="flex items-center gap-2 bg-gray-50 px-4 py-2.5 rounded-xl border border-gray-100 min-w-[240px] group">
                                    ...
                                    <span className="text-xs font-bold text-gray-600 truncate max-w-[150px]">{slug}</span>
                                </Link>
